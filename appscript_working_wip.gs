...
function createPivotSheet(sheet, data_sheet, branch_name, curr_fy_ind) {
  ...
  createPT01(branch_name, 'Branch Summary by Specialist', sum_sheet, drange, 1, 1, 3, "Contract Specialist", fy_values, curr_fy_ind);
  createPT01(branch_name, 'Branch Summary by Client', sum_sheet, drange, 1, 6, 12, "Client", fy_values, curr_fy_ind);
  createPT01(branch_name, 'Branch Summary by Type', sum_sheet, drange, 1, 11, 13, "Type", fy_values, curr_fy_ind);
  createPT02(branch_name, 'Specialist by Client', sum_sheet, drange, 1, 16, 12, "Client", fy_values, curr_fy_ind);
  createPT02(branch_name, 'Type by Specialist', sum_sheet, drange, 1, 21, 13, "Type", fy_values, curr_fy_ind);
  createPT03(branch_name, 'Update by Type by Specialist', sum_sheet, drange, 1, 26, type_values, proc_values, fy_values, curr_fy_ind);
...
function createPT04(branch, title, sheet, data_sheet, data_range, row_position, column_position, filter1, curr_fy_ind) {
  ...
  pivotTable.addFilter(19, criteria);
  // Formatting
  pivotTableCleanup(sheet, row_position, column_position, getUniqueValues(data_sheet, 'B', "", "").length, );
};
...
function cwCreatePivots01(curr_fy_ind) {
  ...
  if (curr_fy_ind != undefined) {
    ...
    cwPivot01("FY" + String(fy) + ' EAD Total WIP', _gac_e, drange, 1, 1, 23, getUniqueValues(_gac_w, 'A', "", ""), pu_values, fy_values, curr_fy_ind, [3, 4, 5]);
    cwPivot01FY("FY" + String(fy + 1) + ' EAD Total WIP', _gac_e, drange, 1, 7, 23, getUniqueValues(_gac_w, 'A', "", ""), pu_values, String(fy + 1), curr_fy_ind, [3, 4, 5]);
    ...
    cwPivot01('EAD Total WIP', _gac_e, drange, 1, 1, 23, getUniqueValues(_gac_w, 'A', "", ""), pu_values, fy_values, curr_fy_ind, [3, 4, 5]);
...
function draftEmail() {
  ...
  //////////////////////////////////////////////////////////////////////
  // Draft Email
  GmailApp.createDraft("email@email.gov, email2@email.gov",
                       "EAD WIP Updated - " + Utilities.formatDate(new Date(), "EST", "M/dd"),
                       "",
                       {cc: "email3@email.gov, email4@email.gov",
